...
(function(window, angular, undefined) {
  ...
  angular.module('satellizer', [])
    ...
    .factory('SatellizerPopup', [
      ...
      'SatellizerConfig',
      'SatellizerUtils',
      function($rootScope, $q, $interval, $window, config, utils) {
        var Popup = {};
        ...
        Popup.pollPopup = function(redirectUri) {
          ...
          var polling = $interval(function() {
            if (!Popup.popupWindow || Popup.popupWindow.closed || Popup.popupWindow.closed === undefined) {
              // console.log('satellizer popup closed normally');
              $rootScope.$broadcast('satellizer:popup:closed');
              $rootScope.$broadcast('satellizer:popup:closed:normal');
            ...
            try {
              ...
              if (popupWindowPath === redirectUriPath) {
                ...
                if (Popup.popupWindow.location.search || Popup.popupWindow.location.hash) {
                  ...
                  if (qs.error) {
                    $rootScope.$broadcast('satellizer:popup:closed');
                    $rootScope.$broadcast('satellizer:popup:closed:error');
                    // console.log('popup error, closing');
                    deferred.reject(qs);
                  } else {
                    $rootScope.$broadcast('satellizer:popup:closed');
                    $rootScope.$broadcast('satellizer:popup:closed:success');
                    // console.log('popup succeeded, closing');
                    deferred.resolve(qs);
                  }
                } else {
                ...
                // console.log('No hash value, poopup closing.');
                $rootScope.$broadcast('satellizer:popup:closed');
                $rootScope.$broadcast('satellizer:popup:closed:nohash');
                $interval.cancel(polling);
                Popup.popupWindow.close();
              }
            } catch (error) {
              $rootScope.$broadcast('satellizer:popup:closed');
              $rootScope.$broadcast('satellizer:popup:closed:domexception');
              // console.log('ignoring dom exception, popup closing.');
