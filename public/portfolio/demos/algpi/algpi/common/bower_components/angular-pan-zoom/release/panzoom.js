/*!
 AngularJS pan/zoom v1.0.17
 @license: MIT
 Github: https://github.com/mvindahl/angular-pan-zoom
*/
/* globals console, jQuery, angular */
...
function ($document, PanZoomService) {
            ...
            return {
                ...
                controller: ['$scope', '$element', '$timeout',
                    function ($scope, $element, $timeout) {
                        ...
                        var calcZoomToFit = function (rect) {
                            ...
                            var zoomLevel = rect.zoom === undefined ? zoomLevelExact * $scope.config.zoomToFitZoomLevelFactor : rect.zoom;
                        ...
                        var setBase = function(base, syncAfter) {
                            if (syncAfter === undefined) {syncAfter = true;}
                            $scope.base = base;
                            if (syncAfter) {
                                console.log('syncing dom');
                                syncModelToDOM();    
                            }
                        };
                        ...
                        var panTo = function(position) {
                            ...
                            $scope.base.pan.x = position.x;
                            $scope.base.pan.y = position.y;
                        ...
                        $scope.onMouseWheel = function ($event, $delta, $deltaX, $deltaY) {
                     	...
                     	var zoomAnimation = function(options) {
                     	    $timeout(function() {
                         	    $scope.zoomAnimation = {
                                    deltaZoomLevel: options.deltaZoomLevel,
                                    translationFromZoom: options.translationFromZoom,
                     	...
                     	var zoomPanTo = function (newZoomLevel, destinationPoint, duration) {
                     	    duration = duration || $scope.config.zoomStepDuration;
                     	    if ($scope.base.zoomLevel !== newZoomLevel) {
                     	        changeZoomLevel( newZoomLevel );
                     	    }
                     	    $timeout(function() {
                     	        this.panTo(getModelPosition(destinationPoint));
                     	    }.bind({panTo: panTo}), duration);
                        };
                ...
                template: '<div class="pan-zoom-frame" ng-dblclick="onDblClick($event)" ng-mousedown="onMousedown($event)"' +
                    ...
                    ' ng-class=\"{ \'zoom-cursor\' : $root.AppConfig.panzoomRaceMap.panZoomInit.zoomOnMouseWheel, ' +
                    '              \'move-cursor\' : !$root.AppConfig.panzoomRaceMap.panZoomInit.zoomOnMouseWheel}\"' +
